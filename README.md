...
### Chrome 下载记录里出现 `file.json`

这表示旧页面请求的临时视频已经因服务重启或到期而被清理，下载接口返回的是错误信息，并不是视频被转换成了 JSON。新版会在保存前检查文件状态与 MP4 类型：文件仍有效时才开始流式下载；失效时会直接在页面提示重新导出。更新代码后请重启 `yarn run dev`，再重新解析一次链接。
